...
## extra stuff
you can also encode intensities objects (unquantized measures) with the coder namespace methods
overloads of `encode` and `decode`. have a look at `<microgl/pixel_coder.h>`